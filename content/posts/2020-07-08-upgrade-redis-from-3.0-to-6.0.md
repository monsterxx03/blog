...
- redis-benchmark 可以加 --cluster 参数工作在 cluster 模式下, 但它会给每个 key 加上 hashtag, 同一个 node 的 key 都会存在同一个 slot 里, 刚开始想用 redis-benchmark 生成测试数据来测试 rebalance 的, 这样就行不通, 只能自己写脚本.
